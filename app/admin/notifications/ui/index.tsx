      ...
      <Card className="lg:col-span-2 border shadow-sm">
        ...
        <CardContent>
          {sentCampaigns.length ? (
            <DataTable headers={["Image", "Titre", "Message", "Destinataires", "Date de diffusion"]}>
              {sentCampaigns.map((camp) => {
                const img = camp.metadata?.imageUrl || (camp as any).imageUrl;
                return (
                  <tr key={camp._id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-3">
                      {img ? (
                        <a href={img} target="_blank" rel="noopener noreferrer" title="Cliquer pour agrandir l'image">
                          <img
                            src={img}
                            alt={camp.title}
                            className="h-10 w-10 rounded-md object-cover border hover:scale-105 transition-transform"
                          />
                        </a>
                      ) : (
