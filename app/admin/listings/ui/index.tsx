...
import { CheckCircle, XCircle, Trash2, Search, MapPin, Eye, X } from "lucide-react";
...
export function AdminListingsClient() {
  ...
  const { data: listingsRes, isLoading } = useQuery({
    queryKey: ["admin-listings", page, search, categoryId, type, status],
    queryFn: async () => {
      ...
      const { data } = await clientApi.get<ListingsResponse>("/listings", { params });
      return data;
    },
  });
  ...
  const moderateMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      const msg = variables.status === "active" ? "Annonce approuvée (activée)" : "Annonce rejetée";
      toast.success(msg);
      // Update modal state if active
      if (selectedListing && selectedListing._id === variables.id) {
        setSelectedListing((prev) => prev ? { ...prev, status: variables.status } : null);
      }
    },
    onError: (error: any) => {
      toast.error(
      ...
      {listingsList.length ? (
        ...
        >
          {listingsList.map((listing) => {
            ...
            const seller = typeof sellerUser === "object" && sellerUser
            ...
            const firstImgUrl = typeof firstImg === "object" && firstImg ? firstImg.url : (typeof firstImg === "string" ? firstImg : null);
            ...
            return (
              <tr key={listing._id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-col">
                    ...
                    <span className="inline-flex items-center gap-1 mt-0.5 text-xxs font-medium text-muted-foreground">
                      <span className={`px-1.5 py-0.5 rounded-full ${listing.listingDirection === "SELL" ? "bg-amber-50 dark:bg-amber-950/20 text-amber-800 dark:text-amber-300" : "bg-blue-50 dark:bg-blue-950/20 text-blue-800 dark:text-blue-300"}`}>
                        {listing.listingDirection === "SELL" ? "Offre" : "Demande"}
                      </span>
                    </span>
                  </div>
                </td>
                ...
                <td className="px-4 py-3 text-sm text-muted-foreground">{seller}</td>
                <td className="px-4 py-3 text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-1.5">
                    {!isApproved && (
                      <Button
                        ...
                        onClick={() => moderateMutation.mutate({ id: listing._id, status: "active" })}
                      >
                    ...
                    {!isRejected && (
                      <Button
                        ...
                        title="Rejeter"
                        disabled={moderateMutation.isPending}
                        onClick={() => moderateMutation.mutate({ id: listing._id, status: "rejected" })}
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 animate-in fade-in duration-200">
          <div className="bg-card border rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between border-b p-4 sticky top-0 bg-card z-10">
              <div>
                <h3 className="font-bold text-lg text-foreground">{selectedListing.title}</h3>
                <span className="text-xs text-muted-foreground font-mono">ID: {selectedListing._id}</span>
              </div>
              <Button variant="ghost" size="icon" onClick={() => setSelectedListing(null)} className="h-8 w-8">
                <X className="h-4 w-4" />
              </Button>
            </div>
            {/* Modal Content */}
            <div className="p-6 space-y-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2 text-sm border-t pt-4">
                <div>
                  ...
                  <span className="font-semibold text-foreground">
                    {typeof selectedListing.categoryId === "object" ? selectedListing.categoryId.name : "-"}
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-xs">Prix</span>
                  <span className="font-bold text-emerald-600 dark:text-emerald-400 text-base">
                    {selectedListing.price} DH
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-xs">Vendeur (Fellah)</span>
                  <span className="font-semibold text-foreground">
                    {typeof selectedListing.sellerId === "object" ? `${selectedListing.sellerId.firstName} ${selectedListing.sellerId.lastName}` : "-"}
                  </span>
                </div>
                <div>
                  ...
                  <span className="font-semibold font-mono text-foreground" dir="ltr">
                    {typeof selectedListing.sellerId === "object" ? selectedListing.sellerId.phone : "-"}
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-xs">Type de transaction</span>
                  <span className="font-semibold text-foreground">
                    {selectedListing.listingDirection === "SELL" ? "Offre (Vente)" : "Demande (Achat)"}
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground block text-xs">Statut actuel</span>
              ...
              <div className="border-t pt-4 space-y-1.5">
                <h4 className="font-semibold text-sm text-foreground">Description de l'annonce</h4>
                ...
                <div className="flex gap-2">
                  {selectedListing.status !== "active" && (
                    <Button
                      ...
                      disabled={moderateMutation.isPending}
                      onClick={() => moderateMutation.mutate({ id: selectedListing._id, status: "active" })}
                    >
                      <CheckCircle className="h-4 w-4" />
                  ...
                  {selectedListing.status !== "rejected" && (
                    <Button
                      ...
                      disabled={moderateMutation.isPending}
                      onClick={() => moderateMutation.mutate({ id: selectedListing._id, status: "rejected" })}
                    >
                      <XCircle className="h-4 w-4" />
